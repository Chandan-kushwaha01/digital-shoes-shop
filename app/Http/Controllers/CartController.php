...
class CartController extends Controller
{
  ...
  public function place_an_order(Request $request)
  {
    ...
    if ($request->mode == 'esewa') {
      $esewa_url = "https://rc-epay.esewa.com.np/api/epay/main/v2/form";

      $amount = number_format($order->total, 2, '.', ''); // convert to NPR if needed
      $tax_amount = number_format($order->tax, 2, '.', '');
      $service_charge = number_format(0, 2, '.', '');
      ...
      $success_url = route('cart.esewa.success');
      $failure_url = route('cart.esewa.fail');

      $signed_field_names = "total_amount,transaction_uuid,product_code";

      // Build signature string in exact order from signed_field_names
      ...
      $raw_signature = hash_hmac('sha256', $signature_string, $secret_key, true);
      ...
      return response()->view('esewa_redirect', compact('esewa_url', 'data'));
    } elseif ($request->mode == 'paypal') {
      //for paypal transaction
    } elseif ($request->mode == 'cod') {
      //for cash on delivery transaction
  ...
  public function esewaSuccess(Request $request)
  {
    ...
    // Optional: You can verify with eSewa here via their Transaction Verification API
    ...
    $transaction->order_id = $order->id;
    $transaction->mode = 'esewa';
    $transaction->status = 'completed';
    $transaction->ref_id = $refId;
    $transaction->save();

    return view('order-confirmation', compact('order'))->with('success', 'Payment successful via eSewa!');
  }
